 ...
 * Claude involved. A 401 means the server enforces per-client OAuth (Authorization
 * on Demand).
 */
async function callAllTools(mcpUrl) {
  ...
  for (const tool of tools) {
    ...
    results.push({
      ...
      output: text || result?.structuredContent || (call.body || "").slice(0, 200),
    });
  }
...
async function createViaConnectionServer(serverName) {
  ...
  try {
    ...
    const execution = await callAllTools(mcpUrl).catch((e) => ({
      ok: false,
      error: e.message,
    }));
    ...
    return {
      ok: true,
      server: { name: serverName, mcpUrl: maskKey(mcpUrl), execution },
    };
  } catch (err) {
