...
/**
 * Catalyst Job Function — runs the full "Authorize via Connection" MCP flow and
 * verifies the server with the Claude Agent SDK (@anthropic-ai/claude-agent-sdk).
 *
 * Ported from the appsail-nodejs AppSail (Claude variant). Runs as a Job (not an
 * HTTP handler) so it can run well beyond the ~15s AppSail request timeout. It:
 *   login -> create server -> add "Catalyst by Zoho LZ" tool (Organization +
 *   List All Projects) -> switch auth to "Authorize via Connection" -> authorize
 *   (OAuth consent tab) -> grab the MCP URL -> Agent SDK query() calls the tools.
 *
 * Verification is Claude-driven via the Agent SDK: we declare the MCP server and
 * the SDK runs the whole agent loop. It runs the MCP client in THIS process (it
 * spawns a local Claude Code subprocess), so an internal host like
 * mcp.localzoho.com is reachable — the Anthropic-hosted MCP connector, which dials
 * out from Anthropic's cloud, could not. Requires ANTHROPIC_API_KEY
 * (ANTHROPIC_MODEL optional).
 *
 * The browser is remote (Catalyst SmartBrowz) via CDP, so only playwright-core is
 * needed — no local Chromium binary.
 *
...
/**
 * Verifies the MCP server using the Claude Agent SDK. We declare the server as a
 * remote HTTP MCP server and the SDK connects, discovers the tools, runs the
 * agent loop, and executes the calls — no hand-rolled transport or tool-use loop.
 * "Authorize via Connection" bakes the key into the MCP URL, so no bearer/header
 * is needed. The Agent SDK runs the MCP client in THIS process (it spawns a local
 * Claude Code subprocess), so an internal host like mcp.localzoho.com is reachable
 ...
 * module) and spawns a bundled Claude Code CLI subprocess. HOME/cwd point at
 * os.tmpdir() (Catalyst fs is read-only except temp); executable is forced to
 * "node" so it doesn't need a bun runtime.
 */
 ...
 * Connection" server: login -> create -> add tools -> switch to "Authorize via
 * Connection" + authorize -> grab MCP URL -> let Claude call the tools to verify.
 */
