...
/**
 * A tiny MCP-over-HTTP (streamable-HTTP) transport. POSTs JSON-RPC, optionally
 * with a Bearer token (required for an on-demand server), captures/echoes the
 * Mcp-Session-Id header, and tolerates responses framed as SSE ("data: {...}"
 * lines) as well as plain JSON.
 */
function makeMcpTransport(mcpUrl, bearer) {
  let sessionId = null;
...
/**
 * Verifies the on-demand MCP server via the Anthropic Messages API MCP CONNECTOR:
 * declare the server (with the OAuth bearer as authorization_token) and Anthropic
 * connects to it and runs the tool loop server-side — the same declarative model
 * as the Agent SDK's mcpServers, but pure JS (no native binary).
 *
 * IMPORTANT: the connection is made from ANTHROPIC'S cloud, so the MCP URL must be
 * publicly reachable. If not, this fails and the caller falls back to
 * callToolsViaClaude() (which connects from this process, carrying the bearer).
 */
async function callToolsViaConnector(mcpUrl, bearer) {
  ...
  const server = { type: "url", name: "catalyst", url: mcpUrl };
  if (bearer) server.authorization_token = bearer;
  const messages = [
    {
      role: "user",
      content:
...
/**
 * FALLBACK: verifies the on-demand MCP server by driving its tools with Claude via
 * the Anthropic SDK (Messages API), proxying each tools/call from THIS process
 * (every request carries the OAuth bearer). Used when the MCP connector can't reach
 * the server from Anthropic's cloud. Tool calls are forced sequential so dependent
 * calls (List All Projects needing an org ID) can reuse earlier results.
 */
async function callToolsViaClaude(mcpUrl, bearer) {
  if (!process.env.ANTHROPIC_API_KEY)
    return { ok: false, error: "ANTHROPIC_API_KEY not set" };
  if (!/^https?:\/\//.test(mcpUrl || ""))
    return { ok: false, error: "no MCP URL captured" };

  const { rpc } = makeMcpTransport(mcpUrl, bearer);
  ...
  if (init.status === 401) {
    return {
      ok: false,
      authRequired: true,
      note: "MCP server returned 401 even with a bearer token — OAuth token invalid/expired.",
    };
  }
...
async function createOnDemandServer(serverName) {
  ...
  try {
    ...
    // runs the tools, using the bearer). Fall back to the local bridge if the
    // connector can't reach the server from Anthropic's cloud.
    console.log(`\nExecuting the tools via the MCP connector on "${serverName}"...`);
    let claudeExecution = await callToolsViaConnector(mcpUrl, auth.accessToken).catch((e) => ({ ok: false, error: e.message }));
    if (!claudeExecution.ok) {
      console.log(
        `  connector unavailable (${claudeExecution.error || "not ok"}); falling back to local bridge...`,
      );
      claudeExecution = await callToolsViaClaude(mcpUrl, auth.accessToken).catch((e) => ({ ok: false, error: e.message }));
    }
    console.log(`  → ${JSON.stringify(claudeExecution)}`);

    return { ok: !!claudeExecution.ok, server: { name: serverName, mcpUrl: maskKey(mcpUrl), claudeExecution } };
  } catch (err) {
